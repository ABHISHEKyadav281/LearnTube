...
const Buttons=styled.button`color:white;width:fit-content;min-width:30%;height:40px;font-size:14px;font-weight:bold; border-radius:30px;border:none;margin-bottom:10px;padding:5px 15px;background-color:#ff4f5a;margin:5px ;`;
...
export default function Signup({darkMode,setDarkMode}) {
  ...
  const handleSignup=async(e)=>{
    e.preventDefault();
    try {
      console.log({name,email,password,country,dob})
      const res=await axios.post("http://localhost:8080/api/auth/signup",{name,email,password,country,dob})
      .then((res)=>{
...
<Box1>
  ...
  <img src="https://img.freepik.com/free-vector/sign-concept-illustration_114360-125.jpg?w=2000" alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
    ...
    <Container>
      ...
      <Buttons onClick={handleSignup}>signup</Buttons>
      <Link to="/login">
      <Buttons style={{backgroundColor:"red"}}>Already have an account</Buttons>
      </Link>
