import React, { useState } from 'react'
import { Link, useNavigate } from 'react-router-dom';
import styled from 'styled-components';
import Upload from './Upload';
import { useSelector } from 'react-redux';
import Logomain from '../utils/logo.png';

const Container = styled.div`
height:7dvh;
width:100%;
position:sticky;
top:0;
padding:8px 12px;
z-index:9;
display:flex;
align-items: center;
  justify-content: space-between;
background-color:${({ theme }) => theme.bgLighter};
color:${({ theme }) => theme.text};
`;

const Search = styled.div`
height:100%;
width:40%;
border:1px solid ${({ theme }) => theme.text};
border-radius:30px;
padding:0 12px;
display:flex;
align-items: center;
  justify-content: space-between;
`;
const Input = styled.input`
height:100%;
display:flex;
align-items: center;
font-size:15px;
width:90%;
outline :none;
border:none;
color:${({ theme }) => theme.text};
background-color:transparent;
`;

const Logo = styled.div`
width:12dvw;margin-top:4px;
height:95%;display:flex;align-Items:center;
`;
const Rightp = styled.div`
width:17%;
height:100%;display:flex;align-Items:center;justify-content: right;gap:25px;font-size:28px;font-weight:400;color:${({ theme }) => theme.text};
`;


const Acc = styled.div`
width:5vh;
height:5vh;
flex-shrink:0;
font-size:32px;
display:flex;align-items:center;justify-content:center;
background-color:RGB(75 199 188);
border-radius:50%;
overflow:hidden;
background-image: url(data:image/png;base64,);
background-size : cover;
color:${({ theme }) => theme.text};overflow:hidden;
`;
const Button = styled.button`
padding:5px 15px;
background-color:transparent;
border:1px solid skyblue;
border-radius:30px;
cursor:pointer;
color:${({ theme }) => theme.text};
`;

export default function Nav({ darkMode, setDarkMode }) {
  const navigate = useNavigate();
  const currentUser = useSelector((state) => state.user);

  const [open, setOpen] = useState(false);
  const [q, setQ] = useState("");


  return (
    <>
    
      <Container>

          

          <Link to="/" style={{ textDecoration: "none" }}>
            <Logo>
              <img src={Logomain} alt="LT" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            </Logo>
          </Link>
        <Search>
          <Input placeholder='Search' onChange={(e) => setQ(e.target.value)}></Input>
          <span class="material-symbols-outlined" style={{ cursor: "pointer" }} onClick={() => { navigate(`/search?q=${q}`) }}>
            search
          </span>
        </Search>

        {currentUser.currentuser ? (
          <Rightp>
            <span onClick={() => { setOpen(true) }} class="material-symbols-outlined" style={{ cursor: "pointer" }}>
              video_call
            </span>
            <span class="material-symbols-outlined" style={{ cursor: "pointer" }}>
              sensors
            </span>
            <Acc><img src={currentUser?.currentuser?.img} alt="" style={{ width: "100%", height: "5dvh", objectFit: "cover", backgroundColor: "transparent" }} /></Acc>
          </Rightp>) : (

          <Link to="signup" style={{ textDecoration: "none" }}>
            <Button>
              Signup
            </Button>
          </Link>)
        }

      </Container>
      {open && <Upload setOpen={setOpen}></Upload>}
    </>
  )
}
